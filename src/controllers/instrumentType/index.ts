...
export default class InstrumentTypeController {
  ...
  configure() {
    ...
    this.router.post('/check',
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        try {
          ...
          console.log(err);
          const errors = ['Failed to check instrument type data.'];
          if (err instanceof Mongoose.Error) {
            errors.push(err.message);
          } else {
            errors.push(err);
    ...
    this.router.post('/remove',
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        try {
          ...
          console.log(err);
          const errors = ['Failed to delete instrument type data.'];
          if (err instanceof Mongoose.Error) {
            errors.push(err.message);
          } else {
            errors.push(err);
    ...
    this.router.get('/', this.authMiddleware, async (req: express.Request, res: express.Response) => {
      try {
        const instruments = await InstrumentType.find().exec();

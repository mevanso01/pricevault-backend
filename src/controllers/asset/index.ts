...
export default class AssetController {
  ...
  configure() {
    ...
    /**
     * POST: check if asset has related instrument types
     */
    this.router.post('/check',
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        try {
          ...
          // Check if the asset has own instrument type
          const related = await InstrumentType.find({ assetId: { $in: items }});
          ...
          console.log(err);
          const errors = ['Failed to check asset data.'];
          if (err instanceof Mongoose.Error) {
            errors.push(err.message);
          } else {
            errors.push(err);
    ...
    this.router.get('/', this.authMiddleware, async (req: express.Request, res: express.Response) => {
      try {
        ...
        console.log(err);
        const errors = ['Failed to fetch asset data.'];
        if (err instanceof Mongoose.Error) {
          errors.push(err.message);
        } else {
          errors.push(err);
